 ...
 * This file has a sibling implementation in scrapers/normalizer.py.
...
/**
 * Price Normalizer: Converts 0, "0", "৳0", "Call for Price", negative to null
 */
export function normalizePrice(rawPrice) {
  if (rawPrice === null || rawPrice === undefined) return null;
  if (typeof rawPrice === 'string' && rawPrice.trim().startsWith('-')) return null;
...
export function extractAttributes(rawTitle) {
  ...
  if (!model) {
    const seriesMatch = title.match(/\b(Vengeance\s*LPX|Vengeance|Fury\s*Renegade|Fury\s*Beast|T-Force\s*Delta|Dominator|Ripjaws|Trident\s*Z|990\s*Pro|980\s*Pro|SN850X|SN770|Blue\s*SN570|P3\s*Plus|P3|NV3|NV2|KC3000|MP600|Barracuda)\b/i);
    if (seriesMatch) {
      model = seriesMatch[1];
    }
  }
...
export function group5StoreOffers(primaryItem, matchingListings = []) {
  ...
  // Automatically assign estimated price for stores marked Call for Price
  finalShops.forEach(s => {
    if (s.price === null || s.price <= 0 || s.price_str === 'Call for Price') {
      s.is_call_for_price = true;
      s.call_for_price_estimated = true;
      s.original_price_str = 'Call for Price';
      if (medianPrice > 0) {
        s.estimated_price = medianPrice;
        s.price = medianPrice;
        s.price_str = `${medianPrice.toLocaleString()}৳`;
  ...
  return {
    best_price: best_price || medianPrice,
    best_price_str: (best_price || medianPrice) ? `${(best_price || medianPrice).toLocaleString()}৳` : 'Call for Price',
    shops: finalShops
  };
}
