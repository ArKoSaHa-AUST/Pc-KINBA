...
class GroqKeyPoolManager {
  ...
  getActiveKey() {
    ...
    // If all keys are currently cooling down, return empty string so callers can fallback gracefully
    return "";
  }
  ...
  markExhausted(key, cooldownMs = 60000) {
    if (!key) return;
    const pool = this.getKeys();
    console.warn(`[Groq Key Manager] Active key #${this.currentIndex + 1}/${pool.length} reached token/rate limit or invalid. Rotating to next key.`);
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      ...
      if (response.status === 429 || response.status === 401 || response.status === 402 || response.status === 403) {
        ...
        console.warn(`[Groq ${response.status === 401 ? 'Invalid Key' : 'Limit'}] HTTP ${response.status} returned by Groq API: ${errText.slice(0, 100)}`);
        const cooldown = response.status === 401 ? 3600000 : 60000;
        groqPool.markExhausted(apiKey, cooldown);
        continue;
      }
