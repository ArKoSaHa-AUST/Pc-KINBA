...
export function deriveCategory(title = "") {
  ...
  if (t.includes("cooler") || t.includes("liquid cooler") || t.includes("cpu cooler") || (t.includes("fan") && !t.includes("dual fan") && !t.includes("triple fan") && !t.includes("graphics card") && !t.includes("edition"))) return "Cooler";
 ...
 * Uses Groq AI keys with Supabase PostgreSQL database catalog.
 */
export async function buildProductAlternatives(targetItem, supabase) {
  ...
  // 1. Try to find other listings with a valid price for the same product in Supabase if price is 0
  if (targetPrice <= 0 && supabase) {
    try {
      const titleWords = targetItem.title.split(/\s+/).filter(w => w.length > 2).slice(0, 2);
      if (titleWords.length > 0) {
        let q = supabase.from("listings").select("price").gt("price", 0);
        for (const w of titleWords) {
          q = q.ilike("title", `%${w}%`);
        }
        const { data: foundPriced } = await q.limit(1);
        if (foundPriced && foundPriced.length > 0 && foundPriced[0].price > 0) {
          targetPrice = foundPriced[0].price;
        }
      }
    } catch (_) {}
  }
  ...
  // 3. Query Supabase for AI-recommended specific models first
  if (aiData && Array.isArray(aiData.alternative_models) && supabase) {
    for (const modelName of aiData.alternative_models) {
      ...
      if (words.length > 0) {
        try {
          let q = supabase.from("listings").select("*").gt("price", 0);
          for (const w of words) {
            q = q.ilike("title", `%${w}%`);
          }
          const { data: rows } = await q.order("price", { ascending: true }).limit(4);
          if (rows) foundRows.push(...rows);
        } catch (_) {}
      }
    }
  }

  // 4. Query Supabase for category bracket candidates
  if (supabase) {
    try {
      let q = supabase.from("listings").select("*").gt("price", 0);
      if (category === "Processor") {
        q = q.or("title.ilike.%processor%,title.ilike.%core i%,title.ilike.%ryzen%,title.ilike.%intel%,title.ilike.%amd%");
      } else if (category === "Graphics Card") {
        q = q.or("title.ilike.%rtx%,title.ilike.%gtx%,title.ilike.%rx %,title.ilike.%graphics card%,title.ilike.%geforce%,title.ilike.%gddr%");
      } else if (category === "Motherboard") {
        q = q.or("title.ilike.%motherboard%,title.ilike.%mainboard%,title.ilike.%b650%,title.ilike.%b760%,title.ilike.%z790%,title.ilike.%b550%");
      } else if (category === "RAM Memory") {
        q = q.or("title.ilike.%ram%,title.ilike.%ddr4%,title.ilike.%ddr5%,title.ilike.%desktop memory%");
      } else if (category === "SSD Storage") {
        q = q.or("title.ilike.%ssd%,title.ilike.%nvme%,title.ilike.%m.2%");
      } else if (category === "Power Supply") {
        q = q.or("title.ilike.%power supply%,title.ilike.%psu%");
      } else if (category === "Monitor") {
        q = q.or("title.ilike.%monitor%,title.ilike.%display%");
      } else if (category === "Laptop") {
        q = q.or("title.ilike.%laptop%,title.ilike.%notebook%,title.ilike.%macbook%");
      }
      const { data: catRows, error } = await q.limit(80);
      if (!error && catRows) {
        foundRows.push(...catRows);
      }
    } catch (err) {
      console.error("[Alternatives Supabase Query Error]:", err.message);
    }
  }
  ...
  for (const row of validRows) {
    ...
    if (!grouped.has(key)) {
      grouped.set(key, {
        ...
        price: row.price,
        oldPrice: Math.round(row.price * (1 + (0.05 + (((row.id || "a").charCodeAt(0) % 8) * 0.01)))),
        discount: 5 + (((row.id || "a").charCodeAt(0) % 8)),
        image: row.image_url || "",
        retailer: row.retailer,
        product_url: row.product_url,
        stores: [row.retailer],
        ratings: 4.5 + (((row.id || "a").charCodeAt(0) % 5) * 0.1),
        reviewsCount: 40 + (((row.id || "ab").charCodeAt(1) % 250))
      });
    } else {
