...
import {
  ...
  getBudgetCeiling as sharedGetBudgetCeiling,
...
/**
 * Returns the allowed budget ceiling (currently budget + 3% tolerance).
 *
 * The tolerance itself is `BUDGET_TOLERANCE` in the shared package — one constant,
 * so making the budget a hard constraint is a single edit rather than a hunt.
 *
 * @param {number} budgetBDT
 * @returns {number}
 */
export function getBudgetCeiling(budgetBDT) {
  return sharedGetBudgetCeiling(budgetBDT);
}
