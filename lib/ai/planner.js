...
/**
 * The extractors now return `undefined` rather than a guess ("UNKNOWN" for sockets,
 * "DDR5" for memory) when a product name is uninformative. Treat an unknown on
 * either side as "could match", which is what the old sentinel comparisons did.
 */
...
export function optimizePlanAlgorithmically(req, candidatesMap, candidateLookup) {
  ...
  for (let cIdx = cpus.length - 1; cIdx >= 0; cIdx--) {
    ...
    const matchingMobos = mobos.filter(m => {
      ...
      return specMatches(cpuSocket, moboSocket);
    });
    ...
    for (let mIdx = matchingMobos.length - 1; mIdx >= 0; mIdx--) {
      ...
      // Find compatible RAM
      const matchingRams = rams.filter(r => specMatches(moboRam, extractRamType(r)));
  ...
  if (!bestBuild) {
    const cpu = cpus[0];
    const cpuSocket = extractSocket(cpu);
    const mobo = mobos.find(m => specMatches(cpuSocket, extractSocket(m))) || mobos[0];
    const moboRam = extractRamType(mobo);
    const ram = rams.find(r => specMatches(moboRam, extractRamType(r))) || rams[0];
