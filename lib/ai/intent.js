...
Analyze the user's message in the context of the ongoing conversation and output ONLY JSON matching:
{
...
- "lakh" / "lac" / "লাখ" = 100000; "k" / "হাজার" = 1000; "cr" / "crore" = 10000000. If no budget specified in this turn, keep the active session budget.
 ...
 */
export function parseIntentRegex(message = "", pinnedConstraints = {}) {
  const text = message.toLowerCase().trim();
  const isBn = /[\u0980-\u09FF]/.test(message);

  // 0. Check Greetings
  if (/^(hi|hello|hey|greetings|howdy|hola|yo|assalamu\s*alaikum|salam|kemon\s*acho|kemon\s*achen|good\s*(morning|afternoon|evening)|হ্যালো|হাই|সালাম|আসসালামু\s*আলাইকুম|কেমন আছেন|কেমন আছো)(?:\s+tonima|\s+তনিমা)?[\s!.,?]*$/i.test(text)) {
    return {
      type: "greeting",
      budget_bdt: pinnedConstraints.budget_bdt || 0,
      purpose: pinnedConstraints.purpose || "general",
      constraints: pinnedConstraints.constraints || {},
      include_peripherals: false,
      language: isBn ? "bn" : (pinnedConstraints.language || "en")
    };
  }
  ...
  const plainNumMatch = text.match(/\b([1-9]\d{4,6})\b/);
  ...
  if (lakhMatch) {
    ...
    budget = parseInt(plainNumMatch[1], 10);
  } else if (pinnedConstraints.budget_bdt > 0) {
    budget = pinnedConstraints.budget_bdt;
  }

  // 3. Purpose extraction
  let purpose = pinnedConstraints.purpose || "general";
  ...
  // 5. Constraints
  const constraints = { ...(pinnedConstraints.constraints || {}) };
  if (purpose === "ai_ml") {
...
/**
 * Parses user message into a validated BuildRequest with conversation context and pinned constraints.
 * 
 * @param {string} message - User natural language query
 * @param {Array<{ role: string, content: string }>} [history=[]] - Conversation context
 * @param {Object} [pinnedConstraints={}] - Pinned constraints from memory
 * @returns {Promise<{ request: import("./schemas.js").BuildRequest, tokensIn: number, tokensOut: number, model: string }>}
 */
export async function parseIntent(message, history = [], pinnedConstraints = {}) {
  if (!message || !message.trim()) {
    return {
      request: BuildRequestSchema.parse({
        ...
        budget_bdt: pinnedConstraints.budget_bdt || 0,
        purpose: pinnedConstraints.purpose || "general",
        language: pinnedConstraints.language || "en"
      }),
      tokensIn: 0,
      tokensOut: 0,
      model: "default"
  ...
  if (/^(hi|hello|hey|greetings|howdy|hola|yo|assalamu\s*alaikum|salam|kemon\s*acho|kemon\s*achen|good\s*(morning|afternoon|evening)|হ্যালো|হাই|সালাম|আসসালামু\s*আলাইকুম|কেমন আছেন|কেমন আছো)[\s!.,?]*$/i.test(trimmedLower)) {
    return {
      request: BuildRequestSchema.parse(parseIntentRegex(message, pinnedConstraints)),
      tokensIn: 0,
      tokensOut: 0,
  ...
  // Include conversation history and pinned constraints
  ...
  try {
    ...
    if (data && typeof data === "object") {
      // Merge pinned constraints if LLM omitted them on follow-up turns
      if ((!data.budget_bdt || data.budget_bdt === 0) && pinnedConstraints.budget_bdt > 0) {
        data.budget_bdt = pinnedConstraints.budget_bdt;
      }
      if ((!data.purpose || data.purpose === "general") && pinnedConstraints.purpose && pinnedConstraints.purpose !== "general") {
        data.purpose = pinnedConstraints.purpose;
      }
      if (pinnedConstraints.constraints && Object.keys(pinnedConstraints.constraints).length > 0) {
        data.constraints = { ...pinnedConstraints.constraints, ...(data.constraints || {}) };
      }
      if (pinnedConstraints.rejected_parts && Object.keys(pinnedConstraints.rejected_parts).length > 0) {
        ...
        data.constraints.rejected_parts = { ...(pinnedConstraints.rejected_parts || {}), ...(data.constraints.rejected_parts || {}) };
      }
      if (pinnedConstraints.locked_parts && Object.keys(pinnedConstraints.locked_parts).length > 0) {
        if (!data.constraints) data.constraints = {};
        data.constraints.locked_parts = { ...(pinnedConstraints.locked_parts || {}), ...(data.constraints.locked_parts || {}) };
      }
      if (!data.language && pinnedConstraints.language) {
        data.language = pinnedConstraints.language;
      }
  ...
  const fallback = parseIntentRegex(message, pinnedConstraints);
  if (pinnedConstraints.rejected_parts && Object.keys(pinnedConstraints.rejected_parts).length > 0) {
    if (!fallback.constraints) fallback.constraints = {};
    fallback.constraints.rejected_parts = { ...pinnedConstraints.rejected_parts };
  }
  if (pinnedConstraints.locked_parts && Object.keys(pinnedConstraints.locked_parts).length > 0) {
    if (!fallback.constraints) fallback.constraints = {};
    fallback.constraints.locked_parts = { ...pinnedConstraints.locked_parts };
  }
  return {
